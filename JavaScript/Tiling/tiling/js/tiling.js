...
var makeGroup = function( colorFunction, shiftFunction, rotationFunction,
  ...
  for( var i = 0; i < tiles.length; i++ ) {
    ...
    for( var j = 0; j <  getRandomInt(3,6) ; j++ ) {
...
var colorFunction = function( x, y ) {
  ...
  var randomColor = function() {
    var red = Math.trunc(128 * Math.random());
    var green = Math.trunc(128 * Math.random());
    var blue = Math.trunc(128 * Math.random());
...
function redraw() {
    xy1 = 1;
    setInterval(redrawchange, 100);
}

function redrawchange() {
    if (x === 1) {
